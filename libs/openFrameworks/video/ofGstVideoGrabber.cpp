...
void ofGstVideoGrabber::listDevices(){
	...
	/////////////////
	// gamuza tweak
	int numInputs = camData.webcam_devices.size();
	
	availableIDs = new int[numInputs];
	availableNames = new string[numInputs];
	// gamuza tweak
	/////////////////
	...
	for(unsigned i=0; i<camData.webcam_devices.size(); i++){
		...
		// gamuza tweak
		char _t[256];
		sprintf(_t,"%s",camData.webcam_devices[i].product_name.c_str());
		//ofLog(OF_LOG_NOTICE,"TESTING : %s\n",_t);
		string temp_name = _t;
		availableIDs[i]=i;
		availableNames[i]=temp_name;
		numAvailableDevices++;
		// gamuza tweak
		/////////////////
...
//----------------------------------------------------------
int ofGstVideoGrabber::getAvailableDevicesNum(){
...
//----------------------------------------------------------
int* ofGstVideoGrabber::getAvailableIDs(){
	
	return availableIDs;
}

//----------------------------------------------------------
string* ofGstVideoGrabber::getAvailableNames(){
