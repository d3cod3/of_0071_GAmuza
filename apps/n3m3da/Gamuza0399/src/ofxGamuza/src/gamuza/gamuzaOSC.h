...
// OSC vars
ofxOscSender			sender;
ofxOscSender			private_sender;
ofxOscReceiver          receiver;

ofxOscMessage           private_message;
...
void gamuzaMain::setupOSC(){
    ...
    private_sender.setup("127.0.0.1",10099);
...
void gamuzaMain::receiveOSC(){
    ...
    while(receiver.hasWaitingMessages()){
        ...
        // check if we have received a new script to render first
        if(received_message.getAddress() == GAMUZA_RECEIVING_SCRIPT_ADDRESS){
            receiveScript(received_message.getArgAsString(0));
        }
        if(received_message.getAddress() == GAMUZA_RECEIVING_SCRIPT_NAME){
            char temp[256];
            
            GAscriptFileName = received_message.getArgAsString(0);
        ...
        for(int i=0;i<oscReceivingLabels.size();i++){
            if(received_message.getAddress() == oscReceivingLabels[i]){
                if(received_message.getArgType(0) == OFXOSC_TYPE_INT32)
					oscReceivingValues[i] = ofToString(received_message.getArgAsInt32(0));
				else if(received_message.getArgType(0) == OFXOSC_TYPE_FLOAT)
					oscReceivingValues[i] = ofToString(received_message.getArgAsFloat(0));
				else if(received_message.getArgType(0) == OFXOSC_TYPE_STRING)
					oscReceivingValues[i] = received_message.getArgAsString(0);
            }
        }
    }
...
void gamuzaMain::receiveScript(string script){
    ...
    liveCoding.pasteFromLuaScript(script);
}
...
void gamuzaMain::resetIncomingOSC(){
    ...
    if(oscReceivingValues.size() > 0){
        oscReceivingValues.clear();
    }
}
...
void gamuzaMain::sendPrivateMessage(int _type, string _message){
    ...
    if(_type == GAMUZA_CONSOLE_LOG){
        private_message.setAddress(GAMUZA_CONSOLE_LOG_ADDRESS);
    }else if(_type == GAMUZA_CONSOLE_ERROR){
        private_message.setAddress(GAMUZA_CONSOLE_ERROR_ADDRESS);
    }
    private_message.addStringArg(_message);
    private_sender.sendMessage(private_message);
}
